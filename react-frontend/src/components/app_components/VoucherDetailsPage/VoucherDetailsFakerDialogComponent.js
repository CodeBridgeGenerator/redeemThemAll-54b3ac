...
import voucherDetailsFakerFactory from "./voucherDetailsFakerFactory";
...
const VoucherDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = voucherDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("voucherDetails").create(elem));
...
export default connect(mapState, mapDispatch)(VoucherDetailsFakerDialogComponent);
