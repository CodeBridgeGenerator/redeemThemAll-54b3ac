...
import userDetailsFakerFactory from "./userDetailsFakerFactory";
...
const UserDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userDetails").create(elem));
...
export default connect(mapState, mapDispatch)(UserDetailsFakerDialogComponent);
